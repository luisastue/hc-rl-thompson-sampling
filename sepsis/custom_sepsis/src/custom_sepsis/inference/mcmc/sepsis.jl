...
module Sepsis
export labeled_categorical, get_reward, random_initial_state, get_next_state, simulate_episode, get_parameters, sepsis_model
...
function hr_probs(parameters, state, action)
    hr_probs = softmax(add_0(
        parameters[:hr=>state.hr=>:intercept]
        .+
        parameters[:hr=>state.hr=>:abx] .* action[:abx]
        .+
        parameters[:hr=>state.hr=>:vaso] .* action[:vaso]
    ))
    return hr_probs
end

function bp_probs(parameters, state, action)
    bp_probs = softmax(add_0(
        parameters[:bp=>state.bp=>:intercept]
        .+
        parameters[:bp=>state.bp=>:abx] .* action[:abx]
        .+
        parameters[:bp=>state.bp=>:vaso] .* action[:vaso]
    ))
    return bp_probs
end

function o2_probs(parameters, state, action)
    o2_probs = softmax(add_0(
        parameters[:o2=>state.o2=>:intercept]
        .+
        parameters[:o2=>state.o2=>:vent] .* action[:vent]
    ))
    return o2_probs
end

function glu_probs(parameters, state, action)
    glu_probs = softmax(add_0(
        parameters[:glu=>state.glu=>:intercept]
        .+
        parameters[:glu=>state.glu=>:vaso] .* action[:vaso]
    ))
    return glu_probs
end

@gen function get_next_state(params::Dict, state::State, action::Action)::State
...
@gen function simulate_episode(beliefs::Dict, policy::Policy, start_state::State)
    states = [start_state]
    state = start_state
    rewards = []
    for t in 1:10
...
@gen function get_parameters()::Dict
    ...
    for level in GLU_LEVELS
        parameters[:glu=>level=>:intercept] =
            {:glu => level => :intercept} ~ mvnormal([0, 0], Diagonal([1.0, 1.0]))
        parameters[:glu=>level=>:vaso] =
            {:glu => level => :vaso} ~ mvnormal([0, 0], Diagonal([1.0, 1.0]))
    end
