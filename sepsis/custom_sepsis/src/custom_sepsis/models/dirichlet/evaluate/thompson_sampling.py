...
class DirThompsonSampling():
    def __init__(self, model: DirModel, models: dict[int, any], policies: dict[int, Policy], mean_rewards: dict[int, float], name: str, info: dict):
        self.name = name
        self.info = info
        self.info["name"] = name
    ...
    def save_json(self, directory="json/dbn/ts"):
        ...
        with open(object_path, 'w') as file:
            json_file = {
                ...
                "mean_rewards": {k: rew for k, rew in self.mean_rewards.items()}
            }
    ...
    def load_json(object_path: str):
        with open(object_path, 'r') as file:
            ...
            return DirThompsonSampling(model, {}, {}, mean_rewards, json_file["info"]["name"], json_file["info"])
